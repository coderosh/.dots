...
local function match_clients(rule, _clients, t, is_excluded)
  ...
  local mf = is_excluded and function(c, _rule)
...
function revelation.expose(args)
  ...
  for scr = 1, capi.screen.count() do
    t[scr] = awful.tag.new({ revelation.tag_name }, scr, awful.layout.suit.fair)[1]
    zt[scr] = awful.tag.new({ revelation.tag_name .. "_zoom" }, scr, awful.layout.suit.fair)[1]
