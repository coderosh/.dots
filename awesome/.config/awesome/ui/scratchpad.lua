-- Copied from https://github.com/BlingCorp/bling
-- https://github.com/BlingCorp/bling/blob/master/LICENSE
...
local function client_turn_off(c, current_tag)
...
local function client_turn_on(c)
...
local function client_is_child_of(c, pid)
...
local function client_find(rule)
...
local function on_animate_turn_off_end(self, tag)
  ...
  client_turn_off(self.client, tag)
  ...
  if not self.client then
    ...
    if capi.awesome.version ~= "v4.3" then
      ...
      local function inital_apply(c1)
        if client_is_child_of(c1, pid) then
          self.client = c1
...
function Scratchpad:toggle()
  ...
  if self.dont_focus_before_close then
    ...
    is_turn_off = capi.client.focus and awful.rules.match(capi.client.focus, self.rule)
  end
...
local sc = setmetatable(Scratchpad, Scratchpad.mt)

local term_scratch = sc({
